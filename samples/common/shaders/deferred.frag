...
vec3 GetL(Light light, vec3 position)
{
	...
	{
		return normalize(light.vec.xyz - position);
	}
}
...
{
	vec3 Lo = BRDF(GetL(light, param.P), param.V, param.N, param.metallic, param.roughness, param.albedo) * light.intensity.xyz;
